...
def _iter_parts(node):
    ...
    2026-08-28 G 盘扫不到的根因：MBR 磁盘的扩展分区（如 sda2）里的逻辑
    分区（sda5/sda6/sda7 = E:/F:/G:）在 lsblk 树里嵌套在扩展容器的
    children 下面，不是磁盘的直接 children。只看一层会把整块机械盘的
    逻辑分区全部漏掉——GPT 盘不受影响（C/D 是直接子分区，所以 D 盘
    扫得到、G 盘扫不到）。这里 DFS 拉平，两种布局都能枚举。
    """
...
def list_disks():
    ...
    for dev in tree:
        ...
        # v2: 递归拉平（MBR 扩展分区里的逻辑分区在二层）
        parts = [as_volume(c) for c in _iter_parts(dev)]
...
def tool_availability():
    ...
    tools = {
        ...
        "mkfs_vfat": "mkfs.vfat",
        "rsync": "rsync",
        # v2.1 2026-08-28：工具页（CPU-Z/AIDA64/硬盘读速/内存读速）needs 里
        # 用了下面这些键，但字典里没有 → 前端 missingDeps 全命中 → 整页
        # 「缺少 xx」灰掉（用户实测反馈：实用工具一个都用不了）。
        "lscpu": "lscpu",
        "dmidecode": "dmidecode",
        "hdparm": "hdparm",
        "mbw": "mbw",
...
def _mount_ro(dev, mnt, err_out=None):
    r"""Read-only mount helper, never raises.

    v3 2026-08-28: 7-level read-only fallback chain for NTFS dirty volumes.
    Level 1 是内核原生 ntfs3（RUN12 已把 ntfs3.ko 注入 squashfs，模块加载
    由 mount/modprobe 自动完成），它对 Fast Startup dirty 卷的容忍度最好；
    后面 6 级是 ntfs-3g（fuse 是内核 builtin）和兜底 mount。
    err_out: 可选 list，把每一级的失败原因写进去（前端 tried 列表展示，
    下次再出「扫描不到」就能看到真实原因而不是干巴巴的"挂载失败"）。
    """
    try:
        os.makedirs(mnt, exist_ok=True)
    except OSError:
        if err_out is not None:
            err_out.append(f"{mnt} mkdir 失败")
        return False
    ...
    def try_mnt(cmd):
        try:
            ...
            ok = os.path.ismount(mnt)
            if not ok and err_out is not None:
                err = (r.stderr or r.stdout or "").strip().splitlines()
                err_out.append(" ".join(cmd[:4]) + ": " +
                               (err[0] if err else "not mounted"))
            return ok
        except Exception as exc:
            if err_out is not None:
                err_out.append(" ".join(cmd[:4]) + ": " + str(exc))
            return False

    if probe_fs == "ntfs":
        # L1: 内核原生 ntfs3（RUN12 注入模块后可用，对 dirty 卷容忍最好）
        if try_mnt(["mount", "-t", "ntfs3", "-o", "ro", dev, mnt]):
            return True
        # L2~L3: ntfs-3g（fuse 内核 builtin）
        ...
        if try_mnt(["ntfs-3g", "-o", "ro,remove_hiberfile,umask=0222,fmask=0222", dev, mnt]):
            return True
        # L4: ntfs3 带强制（部分内核要求 force 才挂 dirty 卷）
    ...
    if probe_fs and probe_fs not in ("", "unknown"):
        if try_mnt(["mount", "-t", probe_fs, "-o", "ro", dev, mnt]):
            return True
    try:
        r = subprocess.run(["mount", "-o", "ro", dev, mnt],
                           capture_output=True, text=True, timeout=20, check=False)
        if not os.path.ismount(mnt) and err_out is not None:
            err = (r.stderr or "").strip().splitlines()
            err_out.append("mount ro: " + (err[0] if err else "not mounted"))
    except Exception:
        pass
...
def find_kla_store():
    ...
    """
    # v4 2026-08-28: 驱动兜底。initramfs 是 MODULES=most，SATA/NVMe 驱动
    # 理论上开机已加载；但万一有冷盘/迟到的设备，这里 modprobe 一把 +
    # udevadm settle 等它出现，比"扫不到"再来回猜强。ntfs3 预加载，
    # mount -t ntfs3 的 request_module 路径就不用走一遍了。
    for mod in ("ahci", "ata_piix", "sd_mod", "ntfs3"):
        run(["modprobe", mod], timeout=8)
    run(["udevadm", "settle", "--timeout=5"], timeout=8)
    ...
    for disk in list_disks():
        for part in disk.get("partitions", []):
            ...
            if not fstype:
                # v3: lsblk 偶尔探测不到未挂载卷的 fstype（多盘/冷盘），
                # 用 blkid 兜底补一次，否则 G 盘这类分区会从 candidates 消失
                dev = "/dev/" + part["name"]
                fstype = run(["blkid", "-s", "TYPE", "-o", "value", dev],
    ...
    for part in candidates:
        ...
        # v3: err_out 把 7 级挂载每级的失败原因记进 tried，前端直接展示
        mount_errs = []
        if not _mount_ro(dev, mnt, err_out=mount_errs):
            # Windows 快速启动/休眠会让卷处于 dirty 状态，挂载可能失败
            detail = "; ".join(mount_errs[:2]) if mount_errs else "挂载失败"
            tried.append(f"{dev} 挂载失败（{detail}）")
            continue
        ...
        # v2.1: count real backup entries as tie-breaker so a volume with
        # hundreds of entries (e.g. G:\KLA) always beats a leftover empty
        # KLA dir with chains=[] on another volume.
        entry_count = 0
        if has_index:
            try:
                with open(os.path.join(store, "index.json"), "r", encoding="utf-8") as fh:
                    data = json.load(fh)
                for c in data.get("chains", []) or []:
                    entry_count += len(c.get("entries", []) or [])
            except Exception:
                entry_count = 0
        hits.append({
            "found": True,
            ...
            "mountpoint": mnt,
            "storePath": store,
            # 排序键：有 index.json 的排前面、条目多的排前面，Windows 系统卷排后面
            "_rank": (0 if has_index else 1, -entry_count, 1 if is_system else 0),
        })
    ...
    best = hits[0]
    best.pop("_rank", None)
    # v2.1: keep full info of EVERY hit (device/label/mountpoint/storePath)
    # so read_backup_index can merge backup chains across all volumes.
    all_stores = []
    for h in hits:
        h.pop("_rank", None)
        all_stores.append({
            "device": h.get("device"),
            "label": h.get("label"),
            "mountpoint": h.get("mountpoint"),
            "storePath": h.get("storePath"),
        })
    best["allStores"] = all_stores
    if len(hits) > 1:
        best["otherCandidates"] = [h["device"] for h in hits[1:]]
    # v4 2026-08-28: 命中时也带回 tried（被跳过卷的挂载失败/无KLA详情）。
    # 之前只有全军覆没才返回 tried——D 盘空 KLA 命中后，G 盘挂载失败的
    # 真实原因就被扔掉了，前端只剩干巴巴的"没有备份链"，没法排查。
    if tried:
        best["tried"] = tried
    return best


def _merge_chains(store, result):
    """v2.1 多卷合并：把所有已挂载 KLA 存储卷的备份链合并进 index.chains。

    用户实测问题（2026-08-28）：备份在 G 盘，Windows 主程序正常读到，
    救援系统却选了 D 盘残留的空 KLA（chains=[]），前端弹
    "index.json 里没有任何备份链"。find_kla_store 的 rank 已按
    entry_count 修正（条目多的卷优先）；本函数是第二层兜底：
    只要任何一个卷上有真实备份链，用户就一定看得到。

    规则：按 chainId 去重合并 best + 其余所有卷的 chains；
    合并出内容后 found=True 并清掉失败 reason；跨卷时附 mergedStores。
    """
    all_stores = store.get("allStores") or []
    if len(all_stores) <= 1:
        return result
    idx = result.get("index") or {}
    merged = []
    seen = set()
    for c in idx.get("chains") or []:
        cid = c.get("chainId")
        if cid in seen:
            continue
        seen.add(cid)
        merged.append(c)
    sources = [store.get("device")] if merged else []
    for other in all_stores:
        sp = other.get("storePath")
        if not sp or sp == store.get("storePath"):
            continue
        ipath = os.path.join(sp, "index.json")
        ...
        try:
            ...
            continue
        if odata.get("schemaVersion") != INDEX_SCHEMA_VERSION:
            continue
        added = 0
        for c in odata.get("chains") or []:
            cid = c.get("chainId")
            if cid in seen:
                continue
            seen.add(cid)
            ...
            added += 1
        if added:
            sources.append(other.get("device"))
    if merged:
        idx = dict(idx)
        idx["chains"] = merged
        result["index"] = idx
...
def read_backup_index():
    ...
    result = {
        ...
        # v4: find_kla_store 命中时也带上被跳过卷的详情（G 盘挂载失败可见）
        "tried": store.get("tried", []),
    }
    ...
    if not os.path.isfile(index_path):
        ...
        result = _merge_chains(store, result)
        return result
...
# 两边改版本时记得同步。
KLA_VERSION = "2.0 Public Beta"


def _cpu_cores_threads():
    """CPU 核心/线程/频率/缓存/架构：/proc/cpuinfo + dmidecode。失败返回空 dict。
    核心 = physical id 去重 × cpu cores 字段；线程 = 全部 processor 行数。
    """
    out = {}
    try:
    ...
    if threads:
        out["threads"] = threads
    # 物理 id 去重（每块 CPU）+ 该块的 cpu cores
    phys = {}
    cur_phys = None
    for line in text.splitlines():
        if line.startswith("physical id\t:"):
            try:
                cur_phys = int(line.split(":", 1)[1].strip())
            except ValueError:
                cur_phys = None
        elif line.startswith("cpu cores\t:") and cur_phys is not None:
            try:
                phys[cur_phys] = int(line.split(":", 1)[1].strip())
            except ValueError:
                pass
    physical_cpus = len(phys) if phys else (1 if threads else 0)
    cores_per = sum(phys.values()) // physical_cpus if physical_cpus and phys else 0
    if not cores_per and threads:
        cores_per = threads
    if physical_cpus:
        out["physicalCpus"] = physical_cpus
        out["cores"] = physical_cpus * cores_per
        out["coresPerCpu"] = cores_per
        if out["cores"] and not out.get("threads"):
            out["threads"] = out["cores"]
        out["hyperthreading"] = (
            "是" if out.get("threads", 0) > out.get("cores", 0)
            else ("否" if out.get("cores", 0) else "—")
        )
    # 基础/睿频：/proc/cpuinfo 的 cpu MHz 当前不算，model name 里若有 @ x.xxGHz
    m_mod = re.search(r"@\s*([0-9.]+)\s*[Gg][Hh][Zz]", text)
    if m_mod:
        out["baseFreq"] = m_mod.group(1) + " GHz"
    # 架构 vendor_id family model stepping cache sizes
    m_v = re.search(r"vendor_id\s*:\s*(\S+)", text)
    if m_v:
        out["vendor"] = {"GenuineIntel": "Intel", "AuthenticAMD": "AMD"}.get(
            m_v.group(1), m_v.group(1))
    m_fam = re.search(r"cpu family\s*:\s*(\d+)", text)
    if m_fam:
        out["family"] = m_fam.group(1)
    m_modl = re.search(r"model\s+:\s*(\d+)", text)
    if m_modl:
        out["modelNo"] = m_modl.group(1)
    m_step = re.search(r"stepping\s*:\s*(\d+)", text)
    if m_step:
        out["stepping"] = m_step.group(1)
    m_flags = re.search(r"flags\s*:\s*(.+)", text)
    if m_flags:
        flags = m_flags.group(1).split()
        # 用户关心的常用指令集挑几个
        pick = []
        for f in ("sse4_2", "avx", "avx2", "avx512f", "bmi2", "sha_ni", "smx", "vmx", "svm"):
            if f in flags:
                pick.append(f.upper().replace("_", "."))
        if pick:
            out["isa"] = "，".join(pick)
    # dmidecode 缓存：3 级
    dmi_cache = run(["dmidecode", "--type", "cache"]) or ""
    for level, key in (("Level 1", "l1"), ("Level 2", "l2"), ("Level 3", "l3")):
        m = re.search(
            level + r"\s*\n[^\n]*\nInstalled Size:\s*(\S+)", dmi_cache, re.MULTILINE)
        if m:
            out[key + "Cache"] = m.group(1)
    # 睿频：dmidecode processor Max Speed
    dmi_cpu = run(["dmidecode", "--type", "processor"]) or ""
    m_cur = re.search(r"Current Speed:\s*(\S+\s*[MmGg][Hh][Zz])", dmi_cpu)
    m_max = re.search(r"Max Speed:\s*(\S+\s*[MmGg][Hh][Zz])", dmi_cpu)
    if m_max and not out.get("boostFreq"):
        out["boostFreq"] = m_max.group(1)
    if m_cur and not out.get("baseFreq"):
        out["baseFreq"] = m_cur.group(1)
    # 主板 + 内存 SPD：直接在 sysinfo 里采，这里不用
    return out


def _memory_slots():
    """dmidecode --type memory → MemoryDevice 逐条，失败返回 [].
    每个 slot: {slot, size, type, speed, manufacturer, part}。
    """
    raw = run(["dmidecode", "--type", "memory"]) or ""
    if not raw:
        return []
    devices = re.split(r"^\s*Memory Device\s*$", raw, flags=re.MULTILINE)[1:]
    out = []
    for dev in devices:
        if re.search(r"^\s*Size:\s*No Module Installed", dev, re.MULTILINE):
            continue
        info = {}
        m = re.search(r"^\s*Size:\s*(.+)", dev, re.MULTILINE)
        if m:
            info["size"] = m.group(1).strip()
        m = re.search(r"^\s*Type:\s*(\S+)", dev, re.MULTILINE)
        if m:
            info["type"] = m.group(1).strip()
        m = re.search(r"^\s*Configured Memory Speed:\s*(.+)", dev, re.MULTILINE)
        if not m:
            m = re.search(r"^\s*Speed:\s*(.+)", dev, re.MULTILINE)
        if m:
            info["speed"] = m.group(1).strip()
        m = re.search(r"^\s*Manufacturer:\s*(.+)", dev, re.MULTILINE)
        if m:
            maker = m.group(1).strip()
            if maker not in ("", "Unknown"):
                info["manufacturer"] = maker
        m = re.search(r"^\s*Part Number:\s*(.+)", dev, re.MULTILINE)
        if m:
            pn = m.group(1).strip()
            if pn not in ("", "None"):
                info["part"] = pn
        m = re.search(r"^\s*Locator:\s*(.+)", dev, re.MULTILINE)
        if m:
            info["slot"] = m.group(1).strip()
        if info:
            out.append(info)
    return out
...
def _board_info():
    """主板品牌/型号/芯片组：dmidecode --type baseboard / --type chipset。"""
    baseboard = run(["dmidecode", "--type", "baseboard"]) or ""
    info = {}
    m = re.search(r"^\s*Manufacturer:\s*(.+)", baseboard, re.MULTILINE)
    if m:
        v = m.group(1).strip()
        if v and v not in ("None", "To be filled by O.E.M."):
            info["boardVendor"] = v
    m = re.search(r"^\s*Product Name:\s*(.+)", baseboard, re.MULTILINE)
    if m:
        v = m.group(1).strip()
        if v and v not in ("None", "Base Board Product Name", "To be filled by O.E.M."):
            info["boardModel"] = v
    m = re.search(r"^\s*Version:\s*(.+)", baseboard, re.MULTILINE)
    if m:
        v = m.group(1).strip()
        if v and v not in ("None", "x.xx", "Rev X.0x"):
            info["boardVersion"] = v
    chip = run(["dmidecode", "--type", "chassis"]) or ""
    m = re.search(r"^\s*Type:\s*(.+)", chip, re.MULTILINE)
    if m:
        info["chassis"] = m.group(1).strip()
    # 芯片组：lspci 00:00.0 Host bridge
    pci = run(["lspci"]) or ""
    for line in pci.splitlines():
        if "Host bridge" in line:
            info["chipset"] = line.split(":", 2)[-1].strip()
            break
    return info


def _audio_devices():
    """音频控制器（lspci 扫 Audio device / Multimedia audio）。"""
    out = []
    for line in (run(["lspci"]) or "").splitlines():
...
def _network_adapters():
    """有线/无线网卡（lspci 扫 Ethernet/Wireless/Network 控制器）。"""
    out = []
    for line in (run(["lspci"]) or "").splitlines():
        if any(k in line for k in ("Ethernet controller", "Network controller",
                                   "Wireless interface")):
            out.append(line.split(":", 2)[-1].strip())
    return out
...
def _battery_info():
    """/sys/class/power_supply/BAT* 读电池信息。失败返回 None。"""
    root = "/sys/class/power_supply"
    try:
        names = [n for n in os.listdir(root) if n.upper().startswith("BAT")]
    except OSError:
        return None
    if not names:
        return None
    ...
    model = _read("model_name") or _read("manufacturer")
    cycles = _read("cycle_count")
    # 容量字段单位按 POWER_SUPPLY_PROP_ENERGY_NOW 或 CHARGE_NOW
    def _wh(field_energy_full, field_energy_now, field_charge_full,
             field_charge_now, field_voltage_now):
        ef = _read(field_energy_full)
        en = _read(field_energy_now)
        cf = _read(field_charge_full)
        cn = _read(field_charge_now)
        vn = _read(field_voltage_now)
        ef_u, en_u, cf_u, cn_u, vn_u = None, None, None, None, None
        try:
            if ef: ef_u = int(ef)
        ...
        # energy 字段单位 μWh → /1e6 Wh；charge 字段单位 μAh → 乘电压(μV)再 /1e12 Wh
        design_wh, cur_wh = None, None
        if ef_u is not None:
            design_wh = ef_u / 1e6
        elif cf_u is not None and vn_u is not None:
            design_wh = cf_u * vn_u / 1e12
        if en_u is not None:
            cur_wh = en_u / 1e6
        elif cn_u is not None and vn_u is not None:
            cur_wh = cn_u * vn_u / 1e12
        return design_wh, cur_wh

    design_wh, cur_wh = _wh(
        "energy_full_design", "energy_now",
        "charge_full_design", "charge_now", "voltage_now")
    # 容量百分比
    cap_pct = None
    c = _read("capacity")
    try:
        ...
        if design_wh and cur_wh:
            cap_pct = 100.0 * cur_wh / design_wh
    # 健康度 = energy_full / energy_full_design
    ef_full = _read("energy_full") or _read("charge_full")
    ef_design = _read("energy_full_design") or _read("charge_full_design")
    health = None
    try:
        if ef_full and ef_design:
            health = round(100.0 * int(ef_full) / int(ef_design), 1)
    ...
    bat = {
        ...
        "tech": tech or None,
        "currentPercent": round(cap_pct, 1) if cap_pct is not None else None,
        "designWh": round(design_wh, 1) if design_wh else None,
        "currentWh": round(cur_wh, 1) if cur_wh else None,
...
def sysinfo_overview():
    ...
    # v2.2 2026-08-28 修复：文本输出 -n 解析在 MODEL 带空格时列错位
    # （"Samsung SSD 970..." MODEL 在 TYPE 前面，split 后 bits[3] 是
    # "SSD" 不是 "disk" → 整行丢弃 → 真实硬盘全被过滤 → 0 磁盘）。
    # 真实磁盘的型号几乎都带空格，所以必须用 lsblk -J JSON 输出。
    # （list_disks() 一直是 -J 所以正常，唯独这里用了文本解析。）
    disks = []
    total_disk_bytes = 0
    raw_lsblk = run([
        "lsblk", "-J", "-b", "-d",
        "-o", "NAME,SIZE,MODEL,TYPE,ROTA"
    ])
    try:
        tree = json.loads(raw_lsblk).get("blockdevices", [])
    ...
    for node in tree:
        ...
        rota = str(node.get("rota", "")).strip()
        size_bytes = int(node.get("size") or 0)
        total_disk_bytes += size_bytes
        disks.append({
            "name": "/dev/" + (node.get("name") or ""),
            "size": _fmt_bytes(size_bytes) if size_bytes else str(node.get("size")),
            "sizeBytes": size_bytes,
            "usedBytes": 0,
            "fsAvailBytes": None,
            "model": model if model and model != "?" else "—",
            "kind": "HDD" if rota == "1" else "SSD/闪存",
            "usePercent": None,
        })
    # 用 list_disks 的各分区 sizeBytes/usedFromFs 给磁盘级 usedBytes 叠加
    try:
        for d0 in list_disks():
            disk_path = "/dev/" + d0.get("name", "")
            target = next((x for x in disks if x["name"] == disk_path), None)
            if not target:
                continue
            used = 0
            for p in d0.get("partitions", []):
                sb = p.get("sizeBytes") or 0
                fs = p.get("freeBytes")
                if fs is None:
                    continue
                used += max(0, sb - fs)
            if target["sizeBytes"] and used:
                target["usedBytes"] = used
                target["usePercent"] = round(100.0 * used / target["sizeBytes"], 1)
    except Exception:
        pass
    ...
    bios_vendor = run(["dmidecode", "-s", "bios-vendor"])
    bios_ver = run(["dmidecode", "-s", "bios-version"])
    board_date = run(["dmidecode", "-s", "baseboard-asset-tag"])  # 不是所有机器有
    # 内存额外：总量 GB / 可用 / 已用 / 缓冲
    mem_used_kb = 0
    mem_avail_kb = 0
    try:
        ...
        buf_kb   = _kb("Buffers")
        cache_kb = _kb("Cached")
        sreclaim_kb = _kb("SReclaimable")
        # 「已用」= Total - Free - Buffers - Cached - SReclaimable（对齐 htop 定义）
        used_kb = max(0, total_kb - free_kb - buf_kb - cache_kb - sreclaim_kb)
        mem_used_kb = used_kb
        mem_avail_kb = avail_kb if avail_kb else free_kb + buf_kb + cache_kb
    except OSError:
        pass

    cpu_detail = _cpu_cores_threads()
    cpu_detail["model"] = cpu_model or "—"
    if not cpu_detail.get("cores") and cores:
        cpu_detail["cores"] = cores

    board = _board_info()
    memory_slots = _memory_slots()

    # 固件+内核+安全启动：顺手从 system_info() 拿一份
    firmware = "UEFI" if os.path.isdir("/sys/firmware/efi") else "Legacy BIOS"
    secure_boot = "未知"
    try:
        for name in os.listdir("/sys/firmware/efi/efivars"):
    ...
    return {
        ...
        "model": model or "—",
        "serial": serial or "—",
        "chassis": board.pop("chassis", None),
        "cpu": cpu_detail,
        "gpus": gpus or ["—"],
        "ramBytes": mem_kb * 1024,
        "ramUsedBytes": mem_used_kb * 1024,
        "ramAvailBytes": mem_avail_kb * 1024,
        "memorySlots": memory_slots,
        "totalDiskBytes": total_disk_bytes,
        "disks": disks,
        "board": board,
        "bios": (bios + (" (" + bios_date + ")" if bios_date else "")).strip() or "—",
        "biosVendor": bios_vendor or "—",
        "biosVersion": bios_ver or "—",
        "biosDate": bios_date or None,
        "firmware": firmware,
        "secureBoot": secure_boot,
        "kernel": run(["uname", "-r"]).strip() or "—",
        "audio": _audio_devices() or [],
        "networkAdapters": _network_adapters() or [],
...
def _fmt_bytes(n):
    ...
    while v >= 1024 and idx < len(units) - 1:
        v /= 1024
        idx += 1
    fmt = "%.1f" if idx >= 2 else "%.0f"
    return (fmt % v).rstrip("0").rstrip(".") + " " + units[idx]


# 使用率采样的上一次 tick，用来算 CPU 空闲差 / 网卡流量差 / 磁盘 IO 差
_METRICS_PREV = {
    "cpu": None,        # (user, nice, system, idle, iowait, irq, sirq, steal)
    "net": None,        # {iface: (rx_bytes, tx_bytes)}
    "disk": None,       # {devpath: (rd_bytes, wr_bytes)}
    "ts": None,         # seconds
}


def _read_cpu_totals():
    """读 /proc/stat 第一行 cpu 的 8 项，返回 tuple。失败返回 None。"""
    try:
    ...
    for p in parts:
        try:
            ...
            vals.append(0)
    # 统一补齐到 8 项（老内核缺 steal/guest 也能算）
    while len(vals) < 8:
        vals.append(0)
    return tuple(vals[:8])
...
def _read_net_counters():
    """/proc/net/dev → {iface: (rx_bytes, tx_bytes)}。跳过 lo / 虚拟接口。"""
    try:
...
def _read_disk_counters():
    """/proc/diskstats → {"/dev/name": (rd_bytes, wr_bytes)}。只保留磁盘本体。"""
    disks_by_name = set()
    for d in list_disks() or []:
        n = d.get("name")
        if n:
            disks_by_name.add(n)
    out = {}
    ...
    for line in lines:
        ...
        if len(bits) < 14:
            continue
        name = bits[2]
        if name not in disks_by_name:
            continue
        try:
            # field 6 rd sectors, field 10 wr sectors → ×512 字节
            rd_sec = int(bits[5])
            wr_sec = int(bits[9])
        except ValueError:
            continue
        out["/dev/" + name] = (rd_sec * 512, wr_sec * 512)
    return out


def live_metrics():
    """「系统概况」图谱的实时数据源：CPU/内存/每盘 IO/网卡 + GPU。
    返回结构：{cpuPct, mem{total,used,avail}, disks[], net[], gpus[], ts}。
    用法：前端轮询 2 秒一次，画任务管理器风格的 5 张卡。
    """
    ...
    if cur and _METRICS_PREV["cpu"] is not None and _METRICS_PREV["ts"]:
        prev = _METRICS_PREV["cpu"]
        prev_total = sum(prev)
        cur_total = sum(cur)
        idle_delta  = (cur[3] + cur[4]) - (prev[3] + prev[4])
        total_delta = cur_total - prev_total
        if total_delta > 0:
            out["cpuPct"] = round(100.0 * (1.0 - idle_delta / total_delta), 1)
    _METRICS_PREV["cpu"] = cur
    ...
    try:
        ...
        mt = ""
    def _mi(key):
        mm = re.search(key + r":\s*(\d+)", mt)
        return int(mm.group(1)) * 1024 if mm else 0
    mem_total = _mi("MemTotal")
    mem_free  = _mi("MemFree")
    mem_avail = _mi("MemAvailable") or (mem_free + _mi("Buffers") + _mi("Cached"))
    mem_buff  = _mi("Buffers") + _mi("Cached") + _mi("SReclaimable")
    mem_used  = max(0, mem_total - mem_free - mem_buff)
    out["mem"] = {
        "total": mem_total,
        "used":  mem_used,
        "avail": mem_avail,
        "pct":   round(100.0 * mem_used / mem_total, 1) if mem_total else 0,
    }

    # ─ 磁盘使用率 + IO 速率 ─
    disk_cur = _read_disk_counters()
    last_ts = _METRICS_PREV["ts"]
    dt = (now - last_ts) if last_ts else 0.0
    out["disks"] = []
    list_d = list_disks() or []
    for d0 in list_d:
        path = "/dev/" + d0.get("name", "")
        # 使用率：各分区 freeBytes 叠加
        used = 0
        cap = d0.get("sizeBytes") or 0
        for p in d0.get("partitions", []):
            sb = p.get("sizeBytes") or 0
            fb = p.get("freeBytes")
            if fb is None:
                continue
            used += max(0, sb - fb)
        use_pct = round(100.0 * used / cap, 1) if cap else None
        # IO 速率
        rd_rate = wr_rate = None
        if disk_cur and _METRICS_PREV["disk"] and dt > 0.2:
            c = disk_cur.get(path)
            p2 = _METRICS_PREV["disk"].get(path)
            if c and p2:
                rd_rate = round(max(0.0, (c[0] - p2[0]) / dt), 0)
                wr_rate = round(max(0.0, (c[1] - p2[1]) / dt), 0)
        out["disks"].append({
            "name": path,
            "model": d0.get("model") or "—",
            "kind": d0.get("kind") or (
                "SSD/闪存" if d0.get("rota") == "0" else "HDD"),
            "totalBytes": cap,
            "usedBytes": used,
            "usePct": use_pct,
            "readBps": rd_rate,
    ...
    _METRICS_PREV["disk"] = disk_cur

    # ─ 网卡流量（按接口上下行速率） ─
    net_cur = _read_net_counters()
    out["net"] = []
    for iface, cur2 in net_cur.items():
        rx_rate = tx_rate = None
        if _METRICS_PREV["net"] and dt > 0.2:
            prev2 = _METRICS_PREV["net"].get(iface)
            if prev2:
                rx_rate = round(max(0.0, (cur2[0] - prev2[0]) / dt), 0)
                tx_rate = round(max(0.0, (cur2[1] - prev2[1]) / dt), 0)
        out["net"].append({
            "iface": iface,
            "rxBps": rx_rate,
            "txBps": tx_rate,
    ...
    _METRICS_PREV["net"] = net_cur

    # ─ GPU 使用率（优先 nvidia-smi，其次 amdgpu 节点 sysfs） ─
    out["gpus"] = []
    ns = run(["nvidia-smi",
              "--query-gpu=name,utilization.gpu,utilization.memory,temperature.gpu",
              "--format=csv,noheader,nounits"], timeout=5) or ""
    if ns:
    ...
    if not out["gpus"]:
        for hwmon in sorted(os.listdir("/sys/class/hwmon")) if os.path.isdir("/sys/class/hwmon") else []:
            d = "/sys/class/hwmon/" + hwmon
            nm_p = os.path.join(d, "name")
            if not os.path.isfile(nm_p):
                continue
            try:
                with open(nm_p, "r", encoding="utf-8") as fh:
                    chip = fh.read().strip()
            except OSError:
                continue
            if chip not in ("amdgpu", "radeon"):
                continue
            def _hwmon_read(fname):
                ...
                try:
                    ...
                    return None
            busy = _hwmon_read("device/gpu_busy_percent")
            temp = _hwmon_read("temp1_input")
            mem  = _hwmon_read("mem_info_vram_total_bytes")
            mem_used = _hwmon_read("mem_info_vram_used_bytes")
            def _val(s, div=1, rnd=1):
                try: v = int(s)
                except (TypeError, ValueError): return None
                return round(v / div, rnd) if div != 1 else v
            mem_pct = None
            if mem and mem_used:
                try:
                    mem_pct = round(100.0 * int(mem_used) / int(mem), 1)
                except (ValueError, ZeroDivisionError):
                    pass
            out["gpus"].append({
                "name": chip.upper() + " 显卡",
                "corePct": _val(busy),
                "memPct":  mem_pct,
                "tempC":   _val(temp, 1000, 1),
            })
            break
    _METRICS_PREV["ts"] = now
    return out
...
def _copy_file(job, src, dst):
    ...
    【修复 P0#2】去掉 dd conv=fsync：每 4MB 一块 fsync = 巨量写放大/慢 5-10x，
                改靠 job_copyout 结尾 run(["sync"]) 全局一次 flush。
    【修复 P1#7】dd iflag 去掉 count_bytes+skip_bytes 单独使用的副作用告警。
    【修复 P2#10】dd stderr 从逐字节 read(1) 改为 4KB 缓冲读 + 手动切 \r/\n。
    """
    try:
        ...
        if size > 16 * 1024 * 1024:
            ...
            cmd = ["dd", f"if={src}", f"of={dst}",
                   "bs=4M", "iflag=fullblock"]
            try:
                proc = _sp.Popen(cmd, stderr=_sp.PIPE, bufsize=0)
            ...
            if proc is not None:
                ...
                while True:
                    ...
                    while i < len(rbuf):
                        ch = rbuf[i]
                        if ch == 0x0D or ch == 0x0A:           # \r or \n
                            line = bytes(rbuf[:i]).decode(
                                "utf-8", errors="replace")
                            # 跳过 \r\n：若 \r 之后紧跟 \n，把 \n 也一次吞
                            if (ch == 0x0D and (i + 1) < len(rbuf)
                                    and rbuf[i + 1] == 0x0A):
                                i += 1
                            # 处理进度行
                            if line and "copied" in line:
                                try:
                                    bdone = int(line.split()[0])
                                    with JOBS_LOCK:
                                        if not hasattr(job, "_dd_last"):
                                            job._dd_last = {}
                                        prev = job._dd_last.get(src, 0)
                                        delta = bdone - prev
                                        if delta > 0:
                                            job.done_bytes += delta
                                        job._dd_last[src] = bdone
                ...
                try:
                    ...
                    pass
                with JOBS_LOCK:
                    last = getattr(job, "_dd_last", {}).get(src, 0)
                    delta = max(0, written - last)
                    if delta > 0:
                        job.done_bytes += delta
                    if getattr(job, "_dd_last", None):
                        try:
                            del job._dd_last[src]
                        except Exception:
                            pass
                if rc != 0 or written != size:
                    try:
                        os.unlink(dst)
                    except OSError:
                        pass
                    with JOBS_LOCK:
                        job.errors.append({"path": src,
                                           "error": f"dd 失败 rc={rc} size_mismatch={written}!={size}"})
                    return False
                try:
                    st = os.stat(src)
        ...
        # --- 默认 / 小文件：Python 4MB 块循环 ---------------------------------
        with open(src, "rb") as fi, open(dst, "wb") as fo:
            # 顺序读提示，减少随机 read 调度
            try:
                os.posix_fadvise(fi.fileno(), 0, 0,
...
def job_copyout(job, src_dev, src_paths, dst_dev, dst_subdir):
    ...
    # 【性能优化 C2 + 修复 P0#1】把任务分成：
    #   · 小文件（≤16 MB 或 size 未知）→ 进 ThreadPoolExecutor 并发；
    #     小文件瓶颈是 syscall+元数据，IO-bound，线程池共享内存让
    #     job.done_bytes / job.errors / job.cancel 真正生效。
    #     （前一版用 ProcessPool：多进程下 job 对象是 pickle 克隆副本，
    #       done_bytes/errors 主线程永远收不到 → 进度卡死、错误丢失。）
    #   · 大文件（>16 MB）→ 保持串行调用 _copy_file，里面用 dd bs=4M 纯吞吐。
    #
    # 并发度：min(4, cpu_count//2)。IO 并发再多 HDD 会 seeks 爆炸。
    small_tasks = []   # list of (src, dst)
    big_tasks = []     # list of (src, dst)
    for src in abs_srcs:
        if job.cancel.is_set():
            break
        if os.path.isdir(src):
            ...
            for root, dirs, files in os.walk(src, followlinks=False):
                ...
                for name in files:
                    ...
                    try:
                        ...
                        sz = 0
                    if sz > 16 * 1024 * 1024:
                        big_tasks.append((s, d))
                    else:
                        small_tasks.append((s, d))
        else:
            ...
            try:
                ...
                sz = 0
            if sz > 16 * 1024 * 1024:
                big_tasks.append((src, d))
            else:
                small_tasks.append((src, d))

    # —— 执行顺序：先小文件（靠并发短时间把数量庞大的小文件消化掉）——
    copied = 0
    try:
    ...
    def _run_one(pair):
        ...
        try:
            ...
            pass
        ok = _copy_file(job, s, d)
        with JOBS_LOCK:
            job.done_files += 1
        return ok

    if _cf is not None and workers > 1 and small_tasks:
        try:
            # 【P0#1】ThreadPoolExecutor：同进程共享 job + JOBS_LOCK + cancel Event
            with _cf.ThreadPoolExecutor(max_workers=workers) as ex:
                futures = []
                for t in small_tasks:
                    if job.cancel.is_set():
                        break
                    futures.append(ex.submit(_run_one, t))
                for f in _cf.as_completed(futures):
                    ...
                    try:
                        ...
                        with JOBS_LOCK:
                            job.errors.append({"path": "(concurrent)",
                                               "error": str(exc)})
        except Exception as exc:
            # 线程池起不来（极少），退化成串行
            Log(f"ThreadPool 启动失败退回串行：{exc}")
            for t in small_tasks:
                if job.cancel.is_set():
                    break
                if _run_one(t):
...
# ---------------------------------------------------------------------------
# 作业：硬盘读速测试（hdparm -T / -t + dd iflag=direct）
# ---------------------------------------------------------------------------

def _parse_hdparm(stdout):
    """解析 hdparm 输出的 Timing 行：
       Timing buffered disk reads: 1532 MB in  3.00 seconds = 510.43 MB/sec
       Timing cached reads:   23412 MB in  1.99 seconds = 11764.56 MB/sec
    返回 {cached_mbs, buffered_mbs} 或 {}。
    """
    out = {}
    for key, reg in (
        ("cached",
         r"Timing cached reads:\s*([0-9.]+)\s*\S+\s+in\s+[0-9.]+\s*\S+\s*=\s*([0-9.]+)\s*MB/sec"),
        ("buffered",
         r"Timing buffered disk reads:\s*([0-9.]+)\s*\S+\s+in\s+[0-9.]+\s*\S+\s*=\s*([0-9.]+)\s*MB/sec"),
    ):
        m = re.search(reg, stdout)
        if m:
            try:
                out[key + "_mbs"] = round(float(m.group(2)), 2)
            except ValueError:
                pass
    return out


def _parse_dd(stdout):
    """解析 dd 结束时的最后一行速度：
       2147483648 bytes (2.1 GB, 2.0 GiB) copied, 8.7 s, 247 MB/s
       或 2147483648 bytes (2,1 GB, 2,0 GiB) kopiert, 8,7 s, 247 MB/s
    优先取 "xxx MB/s" / "xxx GB/s" 尾段浮点。
    """
    m = re.search(r"([0-9]+[.,]?[0-9]*)\s*([kKMGT]?B)/s\s*$", stdout.strip())
    if not m:
        return None
    try:
    ...
    mult = {"B": 1, "KB": 1e3, "MB": 1e6, "GB": 1e9, "TB": 1e12}.get(unit)
    if mult is None:
        return None
    # 统一用 MB/s 对外
    return round(v * mult / 1e6, 2)


def job_hdd_speed(job, dev):
    if not dev or not os.path.exists(dev):
        job.fail(f"设备不存在：{dev}")
        return
    # 合法性：必须在 list_disks() 里
    ok_devs = set("/dev/" + d["name"] for d in list_disks() or [])
    if dev not in ok_devs:
        job.fail(f"设备不在物理磁盘列表中：{dev}")
        return

    steps = [
        ("hdparm -T (缓存读取)",      ["hdparm", "-T", dev], "hdparm_cache"),
        ("hdparm -t (磁盘缓冲读取)",  ["hdparm", "-t", dev], "hdparm_buffered"),
        ("dd 顺序 2GB 直接读取",       ["dd", f"if={dev}", "of=/dev/null",
                                       "bs=1M", "count=2048", "iflag=direct",
                                       "status=progress"], "dd_mbs"),
    ]
    with JOBS_LOCK:
        job.state = "running"
        job.total_files = len(steps)
        job.message = f"硬盘读速测试：{dev}"

    results = {"device": dev, "steps": []}
    for idx, (title, argv, key) in enumerate(steps):
        if job.cancel.is_set():
            job.fail("已取消。")
            return
        with JOBS_LOCK:
            job.current = "> " + " ".join(argv)
            job.message = f"第 {idx+1}/{len(steps)} 步：{title}"
        # 防止 dd 跑 > 1 分钟但没有输出，前端轮询会看 job.current → 用 POLL 周期性挂 lines 到 tail
        proc = subprocess.Popen(
            argv, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True
        )
        out_lines = []
        for line in proc.stdout:
            ...
            out_lines = out_lines[-40:]
            with JOBS_LOCK:
                job.current = line.rstrip()[:140]
        rc = proc.wait()
        tail_all = "\n".join(out_lines)
        with JOBS_LOCK:
            job.done_files = idx + 1

        step_res = {"title": title, "argv": argv,
                    "exitCode": rc, "output": tail_all}
        if rc != 0:
            results["steps"].append(step_res)
            job.fail(f"第 {idx+1} 步（{title}）失败，退出码 {rc}。\n{tail_all}",
                     partial=results)
            return

        if key in ("hdparm_cache", "hdparm_buffered"):
            sub_key = "cached_mbs" if key == "hdparm_cache" else "buffered_mbs"
            parsed = _parse_hdparm(tail_all)
            step_res["metricMBs"] = parsed.get(sub_key)
            step_res["unit"] = "MB/s"
            results[key] = step_res["metricMBs"]
        else:
            mbs = _parse_dd(tail_all)
            step_res["metricMBs"] = mbs
            step_res["unit"] = "MB/s"
            results[key] = mbs
        results["steps"].append(step_res)

    run(["sync"], timeout=30)
    human = (f"缓存读 {results.get('hdparm_cache') or '?'} MB/s · "
             f"磁盘读 {results.get('hdparm_buffered') or '?'} MB/s · "
             f"dd 顺序读 {results.get('dd_mbs') or '?'} MB/s")
    job.finish(f"硬盘读速测试完成：{human}", results=results)
...
def job_ram_speed(job, size_mb, loop):
    size_mb = int(size_mb or 128)
    loop    = int(loop or 3)
    ...
    if shutil.which("mbw") is None:
        job.fail("救援系统缺少 mbw 工具，无法测内存带宽。")
        return

    argv = ["mbw", "-n", str(loop), "-q", str(size_mb)]
    with JOBS_LOCK:
        job.state = "running"
        job.total_files = loop
        job.message = f"内存读速测试：{size_mb} MB × {loop} 次"
    ...
    for line in proc.stdout:
        ln = line.rstrip()
        out_lines.append(ln)
        out_lines = out_lines[-60:]
        with JOBS_LOCK:
            job.current = ln[:140]
            # mbw 每个 METHOD 块输出一行 "AVG\tMethod\t..." 记 done_files
            if ln.startswith("AVG\t"):
                job.done_files = min(loop, job.done_files + 1)
    rc = proc.wait()
    tail_all = "\n".join(out_lines)

    if rc != 0:
        job.fail(f"mbw 退出码 {rc}。\n{tail_all}")
        return

    # mbw 输出 3 个方法的 AVG 行：
    #   AVG     Method: MEMCPY        Elapsed: 0.00897  MiB: 128.00000   Copy: 14264.292 MiB/s
    # 取 Copy 的 M iB/s 值，分别打包三种算法结果 + 平均
    methods = {}
    for ln in out_lines:
        m = re.search(
            r"^AVG\s+Method:\s*(\S+)\s+.*Copy:\s*([0-9.]+)\s*([kKMGT]?i?B)/s", ln)
        if not m:
            continue
        try:
            v = float(m.group(2))
        ...
        if unit in ("KIB", "KB"):
            ...
            v = v * 1024 * 1024
        methods[m.group(1)] = round(v, 2)

    summary = {
        "sizeMB": size_mb,
        "loop": loop,
        "methods": methods,
        "avgMBs": (round(sum(methods.values()) / len(methods), 2)
                   if methods else None),
        "output": tail_all,
    }
    pieces = [f"{k} {v} MB/s" for k, v in methods.items()]
    pieces.append(f"平均 {summary['avgMBs'] or '?'} MB/s")
    job.finish("内存读速测试完成：" + " · ".join(pieces), results=summary)
...
JOB_TYPES = {
    ...
    "hdd_speed": (job_hdd_speed, "硬盘读速"),
    "ram_speed": (job_ram_speed, "内存读速"),
}
...
# ═════════════════════════════════════════════════════════════════
# 实用工具（CPU-Z / AIDA64 / 硬盘读速 / 内存读速）
# 2026-08-28 修复：前端 app.js 一直在调 GET /api/toolbox/{cpuz,aida64,
# hdparm,mbw}，但后端 ROUTES 里从来没有这 4 个端点 → 每次点击都 404 →
# 前端 get() 返回 null → 显示「运行失败」（用户实测反馈：工具一个也
# 不能用）。本节补齐 4 个端点，返回约定与 get() 匹配：HTTP 200 + dict。
# ═════════════════════════════════════════════════════════════════

def _tb_dev_ok(dev):
    """设备名白名单：只放行内核标准磁盘命名，防命令注入。"""
    return bool(re.match(r"^/dev/(sd[a-z]+|nvme\d+n\d+|vd[a-z]+|mmcblk\d+)$",
                         dev or ""))


def toolbox_cpuz():
    """CPU-Z：lscpu + /proc/cpuinfo + dmidecode 全量 + 结构化摘要。"""
    raw = {}
    raw["lscpu"] = run(["lscpu"], timeout=15)
    try:
        with open("/proc/cpuinfo", "r", encoding="utf-8") as fh:
            raw["cpuinfo"] = fh.read()
    except OSError:
    ...
    dmi = {}
    # dmidecode -t 编号：0 BIOS / 2 主板 / 3 机箱 / 4 处理器 / 7 缓存 / 17 内存
    for key, tnum in (("bios", "0"), ("baseboard", "2"), ("chassis", "3"),
                      ("processor", "4"), ("cache", "7"), ("memory", "17")):
        dmi[key] = run(["dmidecode", "-t", tnum], timeout=15)
    raw["dmidecode"] = dmi

    # lscpu 扁平化（前端直接按键取值）
    flat = {}
    for line in raw["lscpu"].splitlines():
        if ":" in line:
            k, v = line.split(":", 1)
            flat[k.strip()] = v.strip()
    try:
        ncpu = int(flat.get("CPU(s)") or "0")
    except ValueError:
        ncpu = 0

    # 内存 SPD：按块拆，抓 Size/Type/Speed/厂商/料号/插槽
    slots = []
    for block in re.split(r"^Handle .*$|^Memory Device\s*$",
                           dmi.get("memory", ""), flags=re.M):
        if not block or "Size:" not in block:
            continue
        def _grab(pat, blk=block):
            m = re.search(pat, blk, re.M)
            return m.group(1).strip() if m else ""
        size = _grab(r"^\s*Size:\s*(.+)$")
        if not size or size.lower().startswith("no module"):
            continue
        slots.append({
            "slot": _grab(r"^\s*(?:Device Locator|Locator):\s*(.+)$") or "DIMM",
            "size": size,
            "type": _grab(r"^\s*Type:\s*(.+)$"),
            "speed": _grab(r"^\s*(?:Configured Clock Speed|Speed):\s*(.+)$"),
            "manufacturer": _grab(r"^\s*Manufacturer:\s*(.+)$"),
            "part": _grab(r"^\s*Part Number:\s*(.+)$"),
        })

    # 主板：dmidecode -t 2 + -t 3 + lspci 主桥当芯片组
    board = {}
    bb = dmi.get("baseboard", "")
    for key, pat in (("boardVendor", r"^\s*Manufacturer:\s*(.+)$"),
                     ("boardModel", r"^\s*Product Name:\s*(.+)$"),
                     ("boardVersion", r"^\s*Version:\s*(.+)$")):
        m = re.search(pat, bb, re.M)
        if m:
            board[key] = m.group(1).strip()
    m = re.search(r"^\s*Type:\s*(.+)$", dmi.get("chassis", ""), re.M)
    if m:
        board["chassis"] = m.group(1).strip()
    for line in run(["lspci"], timeout=15).splitlines():
        if "Host bridge" in line:
            name = line.split(":", 2)[-1].strip()
            if name:
                board["chipset"] = name
...
def toolbox_aida64():
    """AIDA64：lshw + sensors + smartctl -x + usb/net 汇总。"""
    raw = {}
    raw["lshwShort"] = run(["lshw", "-short"], timeout=40)
    raw["lshw"] = run(["lshw"], timeout=60)
    raw["sensors"] = run(["sensors", "-A"], timeout=15)
    raw["upower"] = run(["upower", "--dump"], timeout=15)
    raw["lspci"] = run(["lspci", "-mm"], timeout=15)
    raw["lsusb"] = run(["lsusb"], timeout=15)
    raw["ipLink"] = run(["ip", "-d", "link"], timeout=15)
    raw["ipAddr"] = run(["ip", "addr", "show"], timeout=15)

    # 每块物理盘一张 smartctl -x 标签（smartctl 不在就空列表，前端有提示）
    smarts = []
    if shutil.which("smartctl"):
        for d in list_disks():
            ...
            if not name:
                continue
            try:
                r = subprocess.run(["smartctl", "-x", name],
                                   capture_output=True, text=True,
                                   timeout=90, check=False)
                smarts.append({"name": name, "raw": (r.stdout or "") + (r.stderr or ""),
                               "exitCode": r.returncode})
            except (OSError, subprocess.SubprocessError):
                smarts.append({"name": name, "raw": "smartctl 超时/失败",
                               "exitCode": -1})
    raw["smart"] = smarts
    return {"ok": True, "raw": raw}


def toolbox_hdparm(q):
    """硬盘读速：hdparm -Tt + dd 顺序直读 2GB（全程只读）。"""
    dev = (q.get("dev", [""])[0] or "").strip()
    if not _tb_dev_ok(dev):
        return {"ok": False, "reason": "设备名不合法：%r" % dev}

    steps = []

    def _step(title, argv, pat, timeout):
        mb = None
        try:
            ...
            return None, None, "命令超时（%ds）：%s" % (timeout, " ".join(argv))
        except OSError as exc:
            return None, None, "无法执行：%s" % exc
        m = re.search(pat, out)
        if m:
            try:
                ...
                mb = None
        steps.append({"title": title, "output": out,
                      "metricMBs": mb, "exitCode": rc})
        return mb, rc, out

    # hdparm：解析 "Timing cached reads: ... = 12345.67 MB/sec"
    c_mb, _, err1 = _step("hdparm -T（缓存读）", ["hdparm", "-T", dev],
                          r"=\s*([\d.]+)\s*MB/sec", 60)
    b_mb, _, err2 = _step("hdparm -t（缓冲读）", ["hdparm", "-t", dev],
                          r"=\s*([\d.]+)\s*MB/sec", 60)
    if err1 or err2:
        return {"ok": False, "reason": (err1 or err2)}

    # dd 2GB 直读：解析 stderr 尾行 "... copied, 10.5 s, 203 MB/s"
    d_mb, _, err3 = _step(
        "dd 顺序直读 2GB",
        ["dd", "if=" + dev, "of=/dev/null", "bs=1M", "count=2048",
         "iflag=direct"],
        r"copied.*?,\s*[\d.]+\s*s.*?,\s*([\d.]+)\s*[kMG]B/s", 300)
    if err3:
        return {"ok": False, "reason": err3}
...
def toolbox_mbw(q):
    """内存读速：mbw（先试 -n 语法，失败回退老语法，再失败回退单参）。"""
    try:
        size_mb = max(16, min(1024, int((q.get("size_mb", ["128"])[0]))))
    except ValueError:
        size_mb = 128
    try:
        loop = max(1, min(10, int((q.get("loop", ["3"])[0]))))
    except ValueError:
        loop = 3

    if not shutil.which("mbw"):
        return {"ok": False, "reason": "mbw 未安装",
                "params": {"size_mb": size_mb, "loop": loop}}

    out, argv, rc = "", [], -1
    for cand in (["mbw", "-n", str(loop), str(size_mb)],
                 ["mbw", str(loop), str(size_mb)],
                 ["mbw", str(size_mb)]):
        try:
            ...
            continue
        argv = cand
        if rc == 0 and "AVG" in out:
            break

    if rc != 0 or "AVG" not in out:
        return {"ok": False, "reason": "mbw 运行失败（exit=%d）" % rc,
                "exitCode": rc, "raw": out, "argv": " ".join(argv),
                "params": {"size_mb": size_mb, "loop": loop}}

    # AVG 行：Method: MCMP ... Copy: 1234.567 MiB/s
    summary = {}
    for m in re.finditer(r"AVG\s+Method:\s*(\w+).*?Copy:\s*([\d.]+)\s*(\S+)", out):
        summary[m.group(1)] = {"value": m.group(2), "unit": m.group(3)}
    if not summary:  # 宽松兜底：不带 AVG 前缀的 Method/Copy 行
        for m in re.finditer(r"Method:\s*(\w+).*?Copy:\s*([\d.]+)\s*(\S+)", out):
            summary[m.group(1)] = {"value": m.group(2), "unit": m.group(3)}
...
ROUTES = {
    ...
    "/api/metrics": live_metrics,
    "/api/toolbox/cpuz": toolbox_cpuz,
    "/api/toolbox/aida64": toolbox_aida64,
}
...
QUERY_ROUTES = {
    ...
    "/api/toolbox/hdparm": toolbox_hdparm,
    "/api/toolbox/mbw": toolbox_mbw,
}
