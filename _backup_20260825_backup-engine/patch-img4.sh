#!/bin/bash
# 给 access.img 打补丁 v4：只替换 server.py（拷贝性能优化：dd bs=4M + ProcessPoolExecutor）
# 其它 www 文件 v3 已更新，不动。
set -e
...
WORK=/root/img-patch4
MNT=/mnt/kla-img4
OUT_SQ="$WORK/filesystem-new.squashfs"

echo "v4 patch start (server.py copyout perf)"
...
# 先比一下源 server.py 和镜像里的 server.py MD5，确认真的有差异
